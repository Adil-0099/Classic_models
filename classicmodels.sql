...
#1. Retrieve customer orders with product details (JOIN)
select * from orderdetails limit 1; #ordernumber,productcode
select * from orders limit 1; #ordernumber,#customernumber
select * from products limit 1;#productcode,productname,productline,productdescription,as column
select * from customers limit 1;#customernumber,customername

select c.customername,c.phone,o.customernumber,p.productname,p.productline,p.productdescription,p.quantityinstock,o.orderdate from products p inner join orderdetails od on p.productcode = od.productcode
inner join orders o on o.ordernumber = od.ordernumber
inner join customers c on c.customernumber = o.customernumber order by p.quantityinstock ;

#explanation : As you can see in the output the table contains customer details and the product they purchased with the product details,this way the reader can analyze the data of product sales along with product type.with the hepl of customer's pn number the reader can interact with customer to provide neccessity details of product status instantly with just one call.the date of order column can help the reader o gather info about oldest and newest customer.quantity in stock ensures that the no of quantities left indicating the requirement of product is less or more for ex:if quantity is less the requirement is more which is based on sales and vice versa.
...
group by c.customername,c.customernumber having count(o.ordernumber) > 5;

#explanation--this query helps the reader to analyze customer's name along with the customer's id where the total orders purchased ny that customer is massive and those customers are a reliable asset to the company and these customer's should be on top tier customer list to ensure they get goodies after every 5 orders as a gesture of love inorder to not let go of those customers,resulting in a happy customer who ytusts the company even in loss. 
#3. Get total revenue by product line (JOIN + GROUP BY)
select * from productlines limit 1;#productline,
select * from products limit 1;#productline ,#productcode
select * from orderdetails limit 1;#productcode ,quantityordered * priceeach

select p.productline,sum(od.quantityordered * od.priceeach) as total_revenue from productlines pl inner join products p on pl.productline = p.productline
inner join orderdetails od on od.productcode = p.productcode group by p.productline order by total_revenue desc;

#explanation -- the following query helps the company to analyze details of product line which generated the total revenue amount to the company.on top being the highest indicates that "classic cars" is the most selling product in the company and acts as the primary source of the company.if the clssic cars quantity is less when the requirement is more the company will automatically generate less revenue from the classi car department.with the help of this query u can identify the least revenue generator of the company to make decisions like "how to improve this productline's revenue" and list the down the factors that why does "classic cars " better than "trains" and act accordingly whether to keep listing the low revenue generator from the product line or remove it or may be keep it in the list  hoping that it could surpass the last place in the list based on real time data requirements. 

#4.Creates a view showing total quantity sold and revenue per product.
select * from orderdetails;
select * from products;
drop view view_revenue;

create view view_revenue as (select p.productname,sum(od.quantityordered  ) as total_quantity,sum(od.quantityordered * od.priceeach) as total_revenue from productlines pl inner join products p on pl.productline = p.productline
inner join orderdetails od on od.productcode = p.productcode group by p.productname) ;

select * from view_revenue order by total_revenue desc;

#

#explanation--This data is derived from the view and shared with other departments for analysis. It helps identify which products sell well and how much revenue they generate. With this information, the company can adjust its strategies to maximize revenue.

#For example, if the 1969 Ford Falcon has lower sales than last year and excess inventory remains, the company can introduce discounts or promotions to clear stock quickly. This increases sales, optimizes inventory, and boosts total revenue for the product



#5. Create a temporary table for high-value customers (Temporary Table)
select * from customers;#customernumber,customername
select * from payments;#customernumber,amount

select c.customername,sum(p.amount) as total_amount from customers c
inner join payments p on c.customernumber = p.customernumber group by c.customername;

create temporary table temp_high_value_customer as (select c.customername,sum(p.amount) as total_amount from customers c
inner join payments p on c.customernumber = p.customernumber group by c.customername);

select * from temp_high_value_customer order by total_amount desc;


#explanation -- This temporary table helps analyze customer spending behavior without modifying the original data. Using this data, the company can identify high-value customers who consistently contribute to revenue. These customers can be prioritized during stock clearance or special order handling.

#Additionally, the company can determine low-value customers and develop strategies to increase their engagement. This may include trust-building measures, better service quality, and increased transparency in pricing.

#Evaluating customer value based on total payments rather than quantity ordered is a more reliable metric, as orders can be canceled, but payments made indicate actual revenue.

#6. Retrieve the top 5 customers by order value (JOIN + ORDER BY + LIMIT)

select * from customers limit 1;#customername,#customernumber
select * from orders limit 1;#ordernumber,#customernumber
select * from orderdetails limit 1;#ordernumber,#quantityordered
select sum(od.quantityordered * od.priceeach) from orderdetails od ;

select c.customername,sum(od.quantityordered * od.priceeach) as total_orders_revenue from customers c inner join orders o on c.customernumber = o.customernumber
inner join orderdetails od on od.ordernumber = o.ordernumber group by c.customername order by total_orders_revenue desc limit 5 ;

#explanation--This query retrieves the top 5 customers based on order value, helping the company identify its most valuable clients.

#By analyzing this data, the company can recognize high-revenue customers and ensure their payments are received as expected. Additionally, these top customers are likely regular buyers, making them valuable for long-term business relationships.

#Businesses can use this insight for customer retention strategies, such as offering exclusive deals, festive rewards, or personalized services to strengthen trust. Over time, such customers may develop strong loyalty, sometimes even prioritizing the company's products over market alternatives.

#7 . Find employees who have not made any sales (LEFT JOIN + WHERE NULL)

select * from employees limit 1;#employeenumber,#firstname,#lastname,#officecode
select * from customers limit 1;#customernumber,#postalcode,#salesRepEmployeeNumber
select * from orders limit 1;#customernumber

select concat(e.firstname," ",e.lastname) as employee_name,e.employeenumber,o.ordernumber from employees e left join customers c on e.employeenumber = c.salesRepEmployeeNumber 
left join orders o on o.customernumber = c.customernumber where c.customernumber is null ;

#explanation -- This query retrieves details of employees who have not made any sales. The data can be used to assess employee performance, identifying those who may need training or support to improve their sales skills.Additionally, if an employee consistently shows no improvement, the company can take necessary actions, such as issuing warnings or considering termination.Beyond individual performance, this data also helps the company analyze broader sales trends—for example, identifying market gaps or issues that prevent sales from occurring. By investigating these patterns, the company can refine its sales strategies, enhance customer engagement, and ultimately improve its overall service quality.
